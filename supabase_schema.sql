...
-- يتضمن: حماية تصعيد الصلاحيات، وحماية تضخيم الدرجات، وحماية بنوك الأسئلة
-- ====================================================================
...
-- 3. دالة تفعيل الحساب التلقائي والآمن (تمنع التلاعب بالرتبة أثناء التسجيل)
create or replace function public.handle_new_user()
returns trigger as $$
declare
  v_role text := 'student';
begin
  -- تعيين دور المدير تلقائياً لبريد مالك المنصة فقط
  if new.email = 'abdualaziz.yemen@gmail.com' then
    v_role := 'admin';
  else
    v_role := 'student';
  end if;
...
-- دالة حماية ضد تصعيد الصلاحيات (Prevent Privilege Escalation)
create or replace function public.prevent_role_escalation()
returns trigger as $$
begin
  if new.role <> old.role and not public.is_admin() then
    raise exception 'غير مصرح لك بتعديل صلاحيات الحساب';
  end if;
  return new;
end;
$$ language plpgsql security definer;
...
-- دالة جلب أسئلة الاختبار للطالب دون كشف الإجابات الصحيحة في المتصفح!
...
-- دالة تصحيح الاختبار وتخزين النتيجة في السيرفر دون إمكانية التلاعب أو التكرار
...
begin
  ...
  -- الحماية ضد تكرار الأسئلة لتضخيم الدرجات (Distinct on question_id)
  for v_item in
    select distinct on ((item->>'question_id')) item
